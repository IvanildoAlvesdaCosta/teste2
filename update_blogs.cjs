...
function replaceContent(filepath, newContent, title, description) {
    const content = fs.readFileSync(filepath, 'utf8');
    
    const startStr = '<Layout';
    const startIdx = content.lastIndexOf('---', content.indexOf(startStr));
    
    const endStr = '<aside className="article-sidebar">';
...
<section>
  ...
  <p>
    Utilize um estilete para cortar cuidadosamente as fitas plásticas que prendem a embalagem. Tome cuidado para não danificar as peças internas enquanto realiza o corte. Em seguida, abra a caixa para retirar a bicicleta e seus componentes destinados à montagem, manuseando-os com atenção para evitar arranhões ou amassados. Verifique se todas as peças essenciais estão presentes e em boas condições antes de prosseguir para a montagem. Esse cuidado inicial garante que o processo seguinte ocorra de forma segura e sem contratempos.
  </p>
...
<section>
  ...
  <ol>
    ...
    <li>Identifique o lado correto do disco, que normalmente vem marcado para facilitar a orientation, e posicione-o corretamente na roda.</li>
...
replaceContent("src/pages/blog/teste1.astro", contentTeste1, "Como Montar uma Bicicleta Aro 29: Guia Completo Passo a Passo", "Este artigo detalha o processo completo para montar uma bicicleta aro 29, desde a organização das peças até a regulagem final dos componentes.");
replaceContent("src/pages/blog/teste2.astro", contentTeste2, "Montagem da Bicicleta GTS M1: Passo a Passo Completo", "Aprenda a realizar a montagem completa da bicicleta GTS M1, utilizando as ferramentas necessárias e seguindo um procedimento simples e eficiente.");
replaceContent("src/pages/blog/teste3.astro", contentTeste3, "Como montar uma bike aro 29 direto da fábrica", "Este guia completo apresenta o passo a passo para montar uma bicicleta aro 29 adquirida nova, sem necessidade de levar ao mecânico.");
